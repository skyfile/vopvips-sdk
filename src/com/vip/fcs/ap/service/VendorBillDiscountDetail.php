...
class VendorBillDiscountDetail {
	...
	public $enterPrebilledTotalBillAmount = null;
	...
	public function __construct($vals=null){
		...
		if (!isset(self::$_TSPEC)){
			...
			214 => array(
			'var' => 'enterPrebilledTotalBillAmount'
			),
			215 => array(
			'var' => 'signTime'
			),
		...
		if (is_array($vals)){
			...
			if (isset($vals['enterPrebilledTotalBillAmount'])){
				
				$this->enterPrebilledTotalBillAmount = $vals['enterPrebilledTotalBillAmount'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("enterPrebilledTotalBillAmount" == $schemeField){
				
				$needSkip = false;
				$input->readDouble($this->enterPrebilledTotalBillAmount);
	...
	public function write($output){
		...
		if($this->enterPrebilledTotalBillAmount !== null) {
			
			$xfer += $output->writeFieldBegin('enterPrebilledTotalBillAmount');
			$xfer += $output->writeDouble($this->enterPrebilledTotalBillAmount);
