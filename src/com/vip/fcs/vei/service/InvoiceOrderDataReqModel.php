...
class InvoiceOrderDataReqModel {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['memberEmail'])){
				
				$this->memberEmail = $vals['memberEmail'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("orderItems" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem1 = new \Voop\com\vip\fcs\vei\service\OrderItemsModel();
						$elem1->read($input);
			...
			if ("memberEmail" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->memberEmail);
	...
	public function write($output){
		...
		if($this->memberEmail !== null) {
			...
			$xfer += $output->writeFieldBegin('memberEmail');
			$xfer += $output->writeString($this->memberEmail);
