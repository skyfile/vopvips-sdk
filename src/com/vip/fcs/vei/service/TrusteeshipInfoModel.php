...
class TrusteeshipInfoModel {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['confirmStatus'])){
				
				$this->confirmStatus = $vals['confirmStatus'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("confirmStatus" == $schemeField){
				
				$needSkip = false;
				$input->readByte($this->confirmStatus); 
	...
	public function write($output){
		...
		if($this->trusteeshipPlatform !== null) {
		...
		if($this->confirmStatus !== null) {
			...
			$xfer += $output->writeFieldBegin('confirmStatus');
			$xfer += $output->writeByte($this->confirmStatus);
