...
class OutWmsOrderInfo {
	...
	public function read($input){
		...
		while(true){
			...
			if ("orderDetail" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem0 = new \Voop\com\vip\sce\vlg\osp\wms\service\OutWmsOrderDetail();
						$elem0->read($input);
			...
			if ("orderDeliveryBatch" == $schemeField){
				...
				$needSkip = false;
				$input->readByte($this->orderDeliveryBatch); 
	...
	public function write($output){
		...
		if($this->orderDeliveryBatch !== null) {
			...
			$xfer += $output->writeByte($this->orderDeliveryBatch);
