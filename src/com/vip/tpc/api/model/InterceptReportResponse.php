...
class InterceptReportResponse {
	...
	static $_TSPEC;
	public $header = null;
	public $interceptResponses = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['interceptResponses'])){
				
				$this->interceptResponses = $vals['interceptResponses'];
			}
	...
	public function getName(){
		...
		return 'InterceptReportResponse';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("interceptResponses" == $schemeField){
				
				$needSkip = false;
				
				$this->interceptResponses = array();
				$_size0 = 0;
				$input->readSetBegin();
				while(true){
	...
	public function write($output){
		...
		if($this->interceptResponses !== null) {
			
			$xfer += $output->writeFieldBegin('interceptResponses');
			...
			foreach ($this->interceptResponses as $iter0){
