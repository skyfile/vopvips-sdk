...
class AnormalExpressRequest {
	
	static $_TSPEC;
	public $header = null;
	public $carrierCode = null;
	public $anormalRequest = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['anormalRequest'])){
				
				$this->anormalRequest = $vals['anormalRequest'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("anormalRequest" == $schemeField){
				
				$needSkip = false;
				
				$this->anormalRequest = array();
				$_size1 = 0;
				$input->readSetBegin();
				while(true){
					...
					try{
						...
						$elem1 = new \Voop\com\vip\tpc\api\model\AnormalRequest();
						$elem1->read($input);
						
						$this->anormalRequest[$_size1++] = $elem1;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('anormalRequest');
		
		if (!is_array($this->anormalRequest)){
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
		}
		
		$output->writeSetBegin();
		foreach ($this->anormalRequest as $iter0){
