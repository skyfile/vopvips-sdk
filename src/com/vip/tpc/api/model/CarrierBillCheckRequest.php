...
class CarrierBillCheckRequest {
	...
	public $carrierBillCheckModels = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['carrierBillCheckModels'])){
				
				$this->carrierBillCheckModels = $vals['carrierBillCheckModels'];
			}
	...
	public function getName(){
		...
		return 'CarrierBillCheckRequest';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("carrierBillCheckModels" == $schemeField){
				
				$needSkip = false;
				
				$this->carrierBillCheckModels = array();
				$_size1 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem1 = new \Voop\com\vip\tpc\api\model\CarrierBillCheckModel();
						$elem1->read($input);
						
						$this->carrierBillCheckModels[$_size1++] = $elem1;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('carrierBillCheckModels');
		
		if (!is_array($this->carrierBillCheckModels)){
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
		}
		
		$output->writeListBegin();
		foreach ($this->carrierBillCheckModels as $iter0){
