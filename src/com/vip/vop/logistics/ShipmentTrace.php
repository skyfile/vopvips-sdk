...
class ShipmentTrace {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['coordinate'])){
				
				$this->coordinate = $vals['coordinate'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("extend_fields" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem1 = new \Voop\com\vip\vop\logistics\ExtendField();
						$elem1->read($input);
			...
			if ("coordinate" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->coordinate);
	...
	public function write($output){
		...
		if($this->coordinate !== null) {
			...
			$xfer += $output->writeFieldBegin('coordinate');
			$xfer += $output->writeString($this->coordinate);
