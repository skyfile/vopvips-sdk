...
class CollectionPacketItem {
	
	static $_TSPEC;
	public $barcode = null;
	public $product_name = null;
	public $pre_quantity = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['pre_quantity'])){
				
				$this->pre_quantity = $vals['pre_quantity'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("pre_quantity" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->pre_quantity); 
	...
	public function write($output){
		...
		if($this->pre_quantity !== null) {
			
			$xfer += $output->writeFieldBegin('pre_quantity');
			$xfer += $output->writeI32($this->pre_quantity);
