...
class ShippingInterceptResult {
	
	static $_TSPEC;
	public $logistics_no = null;
	public $intercept_status = null;
	public $msg = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['intercept_status'])){
				
				$this->intercept_status = $vals['intercept_status'];
			}
	...
	public function getName(){
		...
		return 'ShippingInterceptResult';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("intercept_status" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->intercept_status); 
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('intercept_status');
		$xfer += $output->writeI32($this->intercept_status);
