...
class Package {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['traceInfos'])){
				
				$this->traceInfos = $vals['traceInfos'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("traceInfos" == $schemeField){
				
				$needSkip = false;
				
				$this->traceInfos = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem0 = new \Voop\com\vip\vop\omni\logistics\TraceInfo();
						$elem0->read($input);
						
						$this->traceInfos[$_size0++] = $elem0;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->traceInfos !== null) {
			...
			$xfer += $output->writeFieldBegin('traceInfos');
			...
			foreach ($this->traceInfos as $iter0){
