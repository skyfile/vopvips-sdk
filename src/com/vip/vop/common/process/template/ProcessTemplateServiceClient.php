...
class ProcessTemplateServiceClient{
	...
	public static function getService(){
		...
		if (! ProcessTemplateServiceClient::$isRegis){
			
			ProcessTemplateServiceClient::regis ();
			ProcessTemplateServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = ProcessTemplateServiceClient::$DEFAULT_PROXY_IP;
		$port = ProcessTemplateServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\vop\common\process\template\ProcessTemplateServiceHelper();
	}
