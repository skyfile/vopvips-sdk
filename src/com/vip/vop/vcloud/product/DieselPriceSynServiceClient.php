...
class DieselPriceSynServiceClient{
	...
	public static function getService(){
		...
		if (! DieselPriceSynServiceClient::$isRegis){
			
			DieselPriceSynServiceClient::regis ();
			DieselPriceSynServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = DieselPriceSynServiceClient::$DEFAULT_PROXY_IP;
		$port = DieselPriceSynServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\vop\vcloud\product\DieselPriceSynServiceHelper();
	}
