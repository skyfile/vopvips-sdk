...
class FendiProductSynServiceClient{
	...
	public static function getService(){
		...
		if (! FendiProductSynServiceClient::$isRegis){
			
			FendiProductSynServiceClient::regis ();
			FendiProductSynServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = FendiProductSynServiceClient::$DEFAULT_PROXY_IP;
		$port = FendiProductSynServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\vop\vcloud\product\FendiProductSynServiceHelper();
	}
