...
class DieselProductSynServiceClient{
	...
	public static function getService(){
		...
		if (! DieselProductSynServiceClient::$isRegis){
			
			DieselProductSynServiceClient::regis ();
			DieselProductSynServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = DieselProductSynServiceClient::$DEFAULT_PROXY_IP;
		$port = DieselProductSynServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\vop\vcloud\product\DieselProductSynServiceHelper();
	}
