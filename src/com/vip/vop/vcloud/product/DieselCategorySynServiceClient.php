...
class DieselCategorySynServiceClient{
	...
	public static function getService(){
		...
		if (! DieselCategorySynServiceClient::$isRegis){
			
			DieselCategorySynServiceClient::regis ();
			DieselCategorySynServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = DieselCategorySynServiceClient::$DEFAULT_PROXY_IP;
		$port = DieselCategorySynServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\vop\vcloud\product\DieselCategorySynServiceHelper();
	}
