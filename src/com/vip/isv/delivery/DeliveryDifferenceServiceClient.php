...
class DeliveryDifferenceServiceClient{
	...
	public static function getService(){
		...
		if (! DeliveryDifferenceServiceClient::$isRegis){
			
			DeliveryDifferenceServiceClient::regis ();
			DeliveryDifferenceServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = DeliveryDifferenceServiceClient::$DEFAULT_PROXY_IP;
		$port = DeliveryDifferenceServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\isv\delivery\DeliveryDifferenceServiceHelper();
	}
