...
class GetReturnOrderResponse {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['return_orders'])){
				
				$this->return_orders = $vals['return_orders'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("return_orders" == $schemeField){
				
				$needSkip = false;
				
				$this->return_orders = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
	...
	public function write($output){
		...
		if($this->return_orders !== null) {
			
			$xfer += $output->writeFieldBegin('return_orders');
			...
			foreach ($this->return_orders as $iter0){
