...
class ProductPurchaseServiceClient{
	...
	public static function getService(){
		...
		if (! ProductPurchaseServiceClient::$isRegis){
			
			ProductPurchaseServiceClient::regis ();
			ProductPurchaseServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = ProductPurchaseServiceClient::$DEFAULT_PROXY_IP;
		$port = ProductPurchaseServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\xstore\cc\product\pool\service\ProductPurchaseServiceHelper();
	}
