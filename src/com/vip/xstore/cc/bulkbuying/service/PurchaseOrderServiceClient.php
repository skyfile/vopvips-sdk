...
class PurchaseOrderServiceClient{
	...
	public static function getService(){
		...
		if (! PurchaseOrderServiceClient::$isRegis){
			
			PurchaseOrderServiceClient::regis ();
			PurchaseOrderServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = PurchaseOrderServiceClient::$DEFAULT_PROXY_IP;
		$port = PurchaseOrderServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\xstore\cc\bulkbuying\service\PurchaseOrderServiceHelper();
	}
