...
class WopInventoryServiceClient{
	...
	public static function getService(){
		...
		if (! WopInventoryServiceClient::$isRegis){
			
			WopInventoryServiceClient::regis ();
			WopInventoryServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = WopInventoryServiceClient::$DEFAULT_PROXY_IP;
		$port = WopInventoryServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\xstore\inventory\service\wop\WopInventoryServiceHelper();
	}
