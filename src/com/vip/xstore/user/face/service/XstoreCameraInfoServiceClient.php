...
class XstoreCameraInfoServiceClient{
	...
	public static function getService(){
		...
		if (! XstoreCameraInfoServiceClient::$isRegis){
			
			XstoreCameraInfoServiceClient::regis ();
			XstoreCameraInfoServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = XstoreCameraInfoServiceClient::$DEFAULT_PROXY_IP;
		$port = XstoreCameraInfoServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\xstore\user\face\service\XstoreCameraInfoServiceHelper();
	}
