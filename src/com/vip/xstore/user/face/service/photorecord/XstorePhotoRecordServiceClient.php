...
class XstorePhotoRecordServiceClient{
	...
	public static function getService(){
		...
		if (! XstorePhotoRecordServiceClient::$isRegis){
			
			XstorePhotoRecordServiceClient::regis ();
			XstorePhotoRecordServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = XstorePhotoRecordServiceClient::$DEFAULT_PROXY_IP;
		$port = XstorePhotoRecordServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\xstore\user\face\service\photorecord\XstorePhotoRecordServiceHelper();
	}
