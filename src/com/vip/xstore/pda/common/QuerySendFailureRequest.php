...
class QuerySendFailureRequest {
	...
	public function getName(){
		...
		return 'QuerySendFailureRequest';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("start_create_time" == $schemeField){
				...
				$needSkip = false;
				$input->readString($this->start_create_time);
			...
			if ("end_create_time" == $schemeField){
				...
				$needSkip = false;
				$input->readString($this->end_create_time);
	...
	public function write($output){
		...
		if($this->start_create_time !== null) {
			...
			$xfer += $output->writeString($this->start_create_time);
		...
		if($this->end_create_time !== null) {
			...
			$xfer += $output->writeString($this->end_create_time);
