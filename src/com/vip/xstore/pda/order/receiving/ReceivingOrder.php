...
class ReceivingOrder {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['receiving_date'])){
				
				$this->receiving_date = $vals['receiving_date'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("delivery_date" == $schemeField){
				...
				$needSkip = false;
				$input->readString($this->delivery_date);
			...
			if ("action_type" == $schemeField){
				...
				$needSkip = false;
				$input->readByte($this->action_type); 
			...
			if ("receiving_date" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->receiving_date);
	...
	public function write($output){
		...
		$xfer += $output->writeString($this->delivery_date);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('action_type');
		$xfer += $output->writeByte($this->action_type);
		
		$xfer += $output->writeFieldEnd();
		
		if($this->receiving_date !== null) {
			
			$xfer += $output->writeFieldBegin('receiving_date');
			$xfer += $output->writeString($this->receiving_date);
