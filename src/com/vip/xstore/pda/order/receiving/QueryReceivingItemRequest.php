...
class QueryReceivingItemRequest {
	...
	public $box_no = null;
	public $barcode_fuzzy_search = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['barcode_fuzzy_search'])){
				
				$this->barcode_fuzzy_search = $vals['barcode_fuzzy_search'];
			}
	...
	public function getName(){
		...
		return 'QueryReceivingItemRequest';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("barcode_fuzzy_search" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->barcode_fuzzy_search); 
	...
	public function write($output){
		...
		if($this->barcode_fuzzy_search !== null) {
			
			$xfer += $output->writeFieldBegin('barcode_fuzzy_search');
			$xfer += $output->writeI32($this->barcode_fuzzy_search);
