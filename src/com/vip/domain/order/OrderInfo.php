	...
	public function read($input){
		...
		while(true){
			...
			if ("pay_type" == $schemeField){
				...
				$names = \Voop\com\vip\domain\order\OrderPayType::$__names;
				$name = null;
				$input->readString($name);
				foreach ($names as $k => $v){
			...
			if ("transport_day" == $schemeField){
				...
				$names = \Voop\com\vip\domain\order\OrderTransportDay::$__names;
				$name = null;
				$input->readString($name);
				foreach ($names as $k => $v){
			...
			if ("ext_pay_type" == $schemeField){
				...
				$names = \Voop\com\vip\domain\order\OrderExtPayType::$__names;
				$name = null;
				$input->readString($name);
				foreach ($names as $k => $v){
			...
			if ("is_printing_price" == $schemeField){
				...
				$names = \Voop\com\vip\domain\order\OrderIsPrintingPrice::$__names;
				$name = null;
				$input->readString($name);
				foreach ($names as $k => $v){
