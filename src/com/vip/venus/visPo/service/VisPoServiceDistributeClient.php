...
class VisPoServiceDistributeClient{
	...
	public static function getService(){
		...
		if (! VisPoServiceDistributeClient::$isRegis){
			
			VisPoServiceDistributeClient::regis ();
			VisPoServiceDistributeClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = VisPoServiceDistributeClient::$DEFAULT_PROXY_IP;
		$port = VisPoServiceDistributeClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\venus\visPo\service\VisPoServiceDistributeHelper();
	}
