...
class FaceServiceClient{
	...
	public static function getService(){
		...
		if (! FaceServiceClient::$isRegis){
			
			FaceServiceClient::regis ();
			FaceServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = FaceServiceClient::$DEFAULT_PROXY_IP;
		$port = FaceServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\com\vip\arplatform\face\service\FaceServiceHelper();
	}
