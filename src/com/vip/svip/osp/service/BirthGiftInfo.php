...
class BirthGiftInfo {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['prizeList'])){
				
				$this->prizeList = $vals['prizeList'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("birthday" == $schemeField){
				...
				$needSkip = false;
				$input->readString($this->birthday);
	...
	public function write($output){
		...
		if($this->prizeList !== null) {
			...
			$xfer += $output->writeFieldBegin('prizeList');
			...
			foreach ($this->prizeList as $iter0){
