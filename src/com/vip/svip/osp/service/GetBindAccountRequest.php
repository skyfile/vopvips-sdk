...
class GetBindAccountRequest {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['openIdType'])){
				
				$this->openIdType = $vals['openIdType'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("openIdType" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->openIdType); 
	...
	public function write($output){
		...
		if($this->openIdType !== null) {
			...
			$xfer += $output->writeFieldBegin('openIdType');
			$xfer += $output->writeI32($this->openIdType);
