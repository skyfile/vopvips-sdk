...
class InventoryDeductOrdersResponse {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['deductOrders'])){
				
				$this->deductOrders = $vals['deductOrders'];
			}
	...
	public function getName(){
		...
		return 'InventoryDeductOrdersResponse';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("deductOrders" == $schemeField){
				
				$needSkip = false;
				
				$this->deductOrders = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
	...
	public function write($output){
		...
		if($this->deductOrders !== null) {
			
			$xfer += $output->writeFieldBegin('deductOrders');
			...
			foreach ($this->deductOrders as $iter0){
