...
interface MessageServiceIf{
	...
	public function publishMessage( $vendor_id, $vendor_code, $subject, $message_content);
...
class _MessageServiceClient extends \Osp\Base\OspStub implements \vipapis\message\MessageServiceIf{
	...
	public function __construct(){
		...
		parent::__construct("vipapis.message.MessageService", "1.0.0");
	}
	...
	public function publishMessage( $vendor_id, $vendor_code, $subject, $message_content){
		...
		$this->send_publishMessage( $vendor_id, $vendor_code, $subject, $message_content);
		return $this->recv_publishMessage();
	}
...
class MessageService_healthCheck_args {
...
class MessageService_publishMessage_args {
	...
	public function read($input){
		...
		if(true) {
			...
			$input->readString($this->message_content);
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('message_content');
...
class MessageService_healthCheck_result {
...
class MessageService_publishMessage_result {
