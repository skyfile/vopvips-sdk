...
class MessageServiceClient{
	...
	public static function getService(){
		...
		if (! MessageServiceClient::$isRegis){
			
			MessageServiceClient::regis ();
			MessageServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = MessageServiceClient::$DEFAULT_PROXY_IP;
		$port = MessageServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\message\MessageServiceHelper();
	}
