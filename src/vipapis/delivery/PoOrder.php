...
class PoOrder {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['normality_flag'])){
				
				$this->normality_flag = $vals['normality_flag'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("normality_flag" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->normality_flag); 
			...
			if ("unpick_list" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem0 = new \Voop\vipapis\delivery\UnpickInfo();
						$elem0->read($input);
	...
	public function write($output){
		...
		if($this->normality_flag !== null) {
			
			$xfer += $output->writeFieldBegin('normality_flag');
			$xfer += $output->writeI32($this->normality_flag);
