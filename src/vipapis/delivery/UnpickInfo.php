...
class UnpickInfo {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['normal_num'])){
				
				$this->normal_num = $vals['normal_num'];
			}
			
			
			if (isset($vals['pre_num'])){
				
				$this->pre_num = $vals['pre_num'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("normal_num" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->normal_num); 
			...
			if ("pre_num" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->pre_num); 
	...
	public function write($output){
		...
		if($this->normal_num !== null) {
			
			$xfer += $output->writeFieldBegin('normal_num');
			$xfer += $output->writeI32($this->normal_num);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->pre_num !== null) {
			
			$xfer += $output->writeFieldBegin('pre_num');
			$xfer += $output->writeI32($this->pre_num);
