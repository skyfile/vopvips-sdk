...
class FastServiceClient{
	...
	public static function getService(){
		...
		if (! FastServiceClient::$isRegis){
			
			FastServiceClient::regis ();
			FastServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = FastServiceClient::$DEFAULT_PROXY_IP;
		$port = FastServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\finance\FastServiceHelper();
	}
