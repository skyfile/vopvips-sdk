...
class QueryPurchaseOrderReq {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['source_system'])){
				
				$this->source_system = $vals['source_system'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("source_system" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->source_system);
	...
	public function write($output){
		...
		if($this->start_create_time !== null) {
		...
		if($this->source_system !== null) {
			...
			$xfer += $output->writeFieldBegin('source_system');
			$xfer += $output->writeString($this->source_system);
