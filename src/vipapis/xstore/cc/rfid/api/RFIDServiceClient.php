...
class RFIDServiceClient{
	...
	public static function getService(){
		...
		if (! RFIDServiceClient::$isRegis){
			
			RFIDServiceClient::regis ();
			RFIDServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = RFIDServiceClient::$DEFAULT_PROXY_IP;
		$port = RFIDServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\xstore\cc\rfid\api\RFIDServiceHelper();
	}
