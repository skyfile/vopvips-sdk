...
class PriceServiceClient{
	...
	public static function getService(){
		...
		if (! PriceServiceClient::$isRegis){
			
			PriceServiceClient::regis ();
			PriceServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = PriceServiceClient::$DEFAULT_PROXY_IP;
		$port = PriceServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\xstore\cc\price\api\PriceServiceHelper();
	}
