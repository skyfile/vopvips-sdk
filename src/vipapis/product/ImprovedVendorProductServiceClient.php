...
class ImprovedVendorProductServiceClient{
	...
	public static function getService(){
		...
		if (! ImprovedVendorProductServiceClient::$isRegis){
			
			ImprovedVendorProductServiceClient::regis ();
			ImprovedVendorProductServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = ImprovedVendorProductServiceClient::$DEFAULT_PROXY_IP;
		$port = ImprovedVendorProductServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\product\ImprovedVendorProductServiceHelper();
	}
