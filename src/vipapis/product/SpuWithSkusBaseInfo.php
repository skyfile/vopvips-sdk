...
class SpuWithSkusBaseInfo {
	...
	public $status = null;
	public $sku_base_info_list = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['sku_base_info_list'])){
				
				$this->sku_base_info_list = $vals['sku_base_info_list'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("sku_base_info_list" == $schemeField){
				
				$needSkip = false;
				
				$this->sku_base_info_list = array();
				$_size1 = 0;
				$input->readListBegin();
				while(true){
	...
	public function write($output){
		...
		if($this->sku_base_info_list !== null) {
			
			$xfer += $output->writeFieldBegin('sku_base_info_list');
			...
			foreach ($this->sku_base_info_list as $iter0){
