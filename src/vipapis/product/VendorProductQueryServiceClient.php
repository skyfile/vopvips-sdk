...
class VendorProductQueryServiceClient{
	...
	public static function getService(){
		...
		if (! VendorProductQueryServiceClient::$isRegis){
			
			VendorProductQueryServiceClient::regis ();
			VendorProductQueryServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = VendorProductQueryServiceClient::$DEFAULT_PROXY_IP;
		$port = VendorProductQueryServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\product\VendorProductQueryServiceHelper();
	}
