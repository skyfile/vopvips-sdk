...
class CouponServiceClient{
	...
	public static function getService(){
		...
		if (! CouponServiceClient::$isRegis){
			
			CouponServiceClient::regis ();
			CouponServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = CouponServiceClient::$DEFAULT_PROXY_IP;
		$port = CouponServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\vipmax\coupon\CouponServiceHelper();
	}
