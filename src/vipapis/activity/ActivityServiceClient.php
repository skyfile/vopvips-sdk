...
class ActivityServiceClient{
	...
	public static function getService(){
		...
		if (! ActivityServiceClient::$isRegis){
			
			ActivityServiceClient::regis ();
			ActivityServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = ActivityServiceClient::$DEFAULT_PROXY_IP;
		$port = ActivityServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\activity\ActivityServiceHelper();
	}
