<?php


/*
* Copyright (c) 2008-2016 vip.com, All Rights Reserved.
*
* Powered by com.vip.osp.osp-idlc-2.5.11.
*
*/

namespace Voop\vipapis\vsizetable;
interface VendorSizeTableServiceIf{
	
	
	public function addSizeDetail( $vendor_id,\Voop\vipapis\vsizetable\SizeDetailRequest $sizedetail);
	
	public function addSizeTableInfo( $vendor_id,\Voop\vipapis\vsizetable\SizeTableInfoRequest $sizetable);
	
	public function addSizeTableTemplate(\Voop\vipapis\vsizetable\AddSizeTableTemplateRequest $request);
	
	public function deleteSizeDetailByIds( $vendor_id, $sizedetail_ids);
	
	public function deleteSizeTable( $vendor_id, $sizetable_id);
	
	public function deleteSizeTableTemplate( $vendor_id, $template_id);
	
	public function getCategorySizeTableConfigs( $third_category_ids);
	
	public function getDimensionInfos( $template_types);
	
	public function getSizeDetailList( $vendor_id, $sizetable_id);
	
	public function getSizeDetailListByIds( $vendor_id, $sizedetail_ids);
	
	public function getSizeTableInfo( $vendor_id, $sizetable_id);
	
	public function healthCheck();
	
	public function querySizeTableTemplate( $vendor_id, $page, $limit, $sizetable_template_id, $sizetable_template_name);
	
	public function updateSizeDetail( $vendor_id,\Voop\vipapis\vsizetable\SizeDetailRequest $sizedetail);
	
	public function updateSizeTableInfo( $vendor_id,\Voop\vipapis\vsizetable\SizeTableInfoRequest $sizetable);
	
}

class _VendorSizeTableServiceClient extends \Voop\Osp\Base\OspStub implements \Voop\vipapis\vsizetable\VendorSizeTableServiceIf{
	
	public function __construct(){
		
		parent::__construct("vipapis.vsizetable.VendorSizeTableService", "1.0.0");
	}
	
	
	public function addSizeDetail( $vendor_id,\Voop\vipapis\vsizetable\SizeDetailRequest $sizedetail){
		
		$this->send_addSizeDetail( $vendor_id, $sizedetail);
		return $this->recv_addSizeDetail();
	}
	
	public function send_addSizeDetail( $vendor_id,\Voop\vipapis\vsizetable\SizeDetailRequest $sizedetail){
		
		$this->initInvocation("addSizeDetail");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_addSizeDetail_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizedetail = $sizedetail;
		
		$this->send_base($args);
	}
	
	public function recv_addSizeDetail(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_addSizeDetail_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function addSizeTableInfo( $vendor_id,\Voop\vipapis\vsizetable\SizeTableInfoRequest $sizetable){
		
		$this->send_addSizeTableInfo( $vendor_id, $sizetable);
		return $this->recv_addSizeTableInfo();
	}
	
	public function send_addSizeTableInfo( $vendor_id,\Voop\vipapis\vsizetable\SizeTableInfoRequest $sizetable){
		
		$this->initInvocation("addSizeTableInfo");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_addSizeTableInfo_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizetable = $sizetable;
		
		$this->send_base($args);
	}
	
	public function recv_addSizeTableInfo(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_addSizeTableInfo_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function addSizeTableTemplate(\Voop\vipapis\vsizetable\AddSizeTableTemplateRequest $request){
		
		$this->send_addSizeTableTemplate( $request);
		return $this->recv_addSizeTableTemplate();
	}
	
	public function send_addSizeTableTemplate(\Voop\vipapis\vsizetable\AddSizeTableTemplateRequest $request){
		
		$this->initInvocation("addSizeTableTemplate");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_addSizeTableTemplate_args();
		
		$args->request = $request;
		
		$this->send_base($args);
	}
	
	public function recv_addSizeTableTemplate(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_addSizeTableTemplate_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function deleteSizeDetailByIds( $vendor_id, $sizedetail_ids){
		
		$this->send_deleteSizeDetailByIds( $vendor_id, $sizedetail_ids);
		return $this->recv_deleteSizeDetailByIds();
	}
	
	public function send_deleteSizeDetailByIds( $vendor_id, $sizedetail_ids){
		
		$this->initInvocation("deleteSizeDetailByIds");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_deleteSizeDetailByIds_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizedetail_ids = $sizedetail_ids;
		
		$this->send_base($args);
	}
	
	public function recv_deleteSizeDetailByIds(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_deleteSizeDetailByIds_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function deleteSizeTable( $vendor_id, $sizetable_id){
		
		$this->send_deleteSizeTable( $vendor_id, $sizetable_id);
		return $this->recv_deleteSizeTable();
	}
	
	public function send_deleteSizeTable( $vendor_id, $sizetable_id){
		
		$this->initInvocation("deleteSizeTable");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_deleteSizeTable_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizetable_id = $sizetable_id;
		
		$this->send_base($args);
	}
	
	public function recv_deleteSizeTable(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_deleteSizeTable_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function deleteSizeTableTemplate( $vendor_id, $template_id){
		
		$this->send_deleteSizeTableTemplate( $vendor_id, $template_id);
		return $this->recv_deleteSizeTableTemplate();
	}
	
	public function send_deleteSizeTableTemplate( $vendor_id, $template_id){
		
		$this->initInvocation("deleteSizeTableTemplate");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_deleteSizeTableTemplate_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->template_id = $template_id;
		
		$this->send_base($args);
	}
	
	public function recv_deleteSizeTableTemplate(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_deleteSizeTableTemplate_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function getCategorySizeTableConfigs( $third_category_ids){
		
		$this->send_getCategorySizeTableConfigs( $third_category_ids);
		return $this->recv_getCategorySizeTableConfigs();
	}
	
	public function send_getCategorySizeTableConfigs( $third_category_ids){
		
		$this->initInvocation("getCategorySizeTableConfigs");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_getCategorySizeTableConfigs_args();
		
		$args->third_category_ids = $third_category_ids;
		
		$this->send_base($args);
	}
	
	public function recv_getCategorySizeTableConfigs(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_getCategorySizeTableConfigs_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function getDimensionInfos( $template_types){
		
		$this->send_getDimensionInfos( $template_types);
		return $this->recv_getDimensionInfos();
	}
	
	public function send_getDimensionInfos( $template_types){
		
		$this->initInvocation("getDimensionInfos");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_getDimensionInfos_args();
		
		$args->template_types = $template_types;
		
		$this->send_base($args);
	}
	
	public function recv_getDimensionInfos(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_getDimensionInfos_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function getSizeDetailList( $vendor_id, $sizetable_id){
		
		$this->send_getSizeDetailList( $vendor_id, $sizetable_id);
		return $this->recv_getSizeDetailList();
	}
	
	public function send_getSizeDetailList( $vendor_id, $sizetable_id){
		
		$this->initInvocation("getSizeDetailList");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_getSizeDetailList_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizetable_id = $sizetable_id;
		
		$this->send_base($args);
	}
	
	public function recv_getSizeDetailList(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_getSizeDetailList_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function getSizeDetailListByIds( $vendor_id, $sizedetail_ids){
		
		$this->send_getSizeDetailListByIds( $vendor_id, $sizedetail_ids);
		return $this->recv_getSizeDetailListByIds();
	}
	
	public function send_getSizeDetailListByIds( $vendor_id, $sizedetail_ids){
		
		$this->initInvocation("getSizeDetailListByIds");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_getSizeDetailListByIds_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizedetail_ids = $sizedetail_ids;
		
		$this->send_base($args);
	}
	
	public function recv_getSizeDetailListByIds(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_getSizeDetailListByIds_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function getSizeTableInfo( $vendor_id, $sizetable_id){
		
		$this->send_getSizeTableInfo( $vendor_id, $sizetable_id);
		return $this->recv_getSizeTableInfo();
	}
	
	public function send_getSizeTableInfo( $vendor_id, $sizetable_id){
		
		$this->initInvocation("getSizeTableInfo");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_getSizeTableInfo_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizetable_id = $sizetable_id;
		
		$this->send_base($args);
	}
	
	public function recv_getSizeTableInfo(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_getSizeTableInfo_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function healthCheck(){
		
		$this->send_healthCheck();
		return $this->recv_healthCheck();
	}
	
	public function send_healthCheck(){
		
		$this->initInvocation("healthCheck");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_healthCheck_args();
		
		$this->send_base($args);
	}
	
	public function recv_healthCheck(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_healthCheck_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function querySizeTableTemplate( $vendor_id, $page, $limit, $sizetable_template_id, $sizetable_template_name){
		
		$this->send_querySizeTableTemplate( $vendor_id, $page, $limit, $sizetable_template_id, $sizetable_template_name);
		return $this->recv_querySizeTableTemplate();
	}
	
	public function send_querySizeTableTemplate( $vendor_id, $page, $limit, $sizetable_template_id, $sizetable_template_name){
		
		$this->initInvocation("querySizeTableTemplate");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_querySizeTableTemplate_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->page = $page;
		
		$args->limit = $limit;
		
		$args->sizetable_template_id = $sizetable_template_id;
		
		$args->sizetable_template_name = $sizetable_template_name;
		
		$this->send_base($args);
	}
	
	public function recv_querySizeTableTemplate(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_querySizeTableTemplate_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function updateSizeDetail( $vendor_id,\Voop\vipapis\vsizetable\SizeDetailRequest $sizedetail){
		
		$this->send_updateSizeDetail( $vendor_id, $sizedetail);
		return $this->recv_updateSizeDetail();
	}
	
	public function send_updateSizeDetail( $vendor_id,\Voop\vipapis\vsizetable\SizeDetailRequest $sizedetail){
		
		$this->initInvocation("updateSizeDetail");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_updateSizeDetail_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizedetail = $sizedetail;
		
		$this->send_base($args);
	}
	
	public function recv_updateSizeDetail(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_updateSizeDetail_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
	public function updateSizeTableInfo( $vendor_id,\Voop\vipapis\vsizetable\SizeTableInfoRequest $sizetable){
		
		$this->send_updateSizeTableInfo( $vendor_id, $sizetable);
		return $this->recv_updateSizeTableInfo();
	}
	
	public function send_updateSizeTableInfo( $vendor_id,\Voop\vipapis\vsizetable\SizeTableInfoRequest $sizetable){
		
		$this->initInvocation("updateSizeTableInfo");
		$args = new \Voop\vipapis\vsizetable\VendorSizeTableService_updateSizeTableInfo_args();
		
		$args->vendor_id = $vendor_id;
		
		$args->sizetable = $sizetable;
		
		$this->send_base($args);
	}
	
	public function recv_updateSizeTableInfo(){
		
		$result = new \Voop\vipapis\vsizetable\VendorSizeTableService_updateSizeTableInfo_result();
		$this->receive_base($result);
		if ($result->success !== null){
			
			return $result->success;
		}
		
	}
	
	
}




class VendorSizeTableService_addSizeDetail_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizedetail = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizedetail'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizedetail'])){
				
				$this->sizedetail = $vals['sizedetail'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			
			$this->sizedetail = new \Voop\vipapis\vsizetable\SizeDetailRequest();
			$this->sizedetail->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizedetail');
		
		if (!is_object($this->sizedetail)) {
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
		}
		
		$xfer += $this->sizedetail->write($output);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_addSizeTableInfo_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizetable = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizetable'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizetable'])){
				
				$this->sizetable = $vals['sizetable'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			
			$this->sizetable = new \Voop\vipapis\vsizetable\SizeTableInfoRequest();
			$this->sizetable->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizetable');
		
		if (!is_object($this->sizetable)) {
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
		}
		
		$xfer += $this->sizetable->write($output);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_addSizeTableTemplate_args {
	
	static $_TSPEC;
	public $request = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'request'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['request'])){
				
				$this->request = $vals['request'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->request = new \Voop\vipapis\vsizetable\AddSizeTableTemplateRequest();
			$this->request->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('request');
		
		if (!is_object($this->request)) {
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
		}
		
		$xfer += $this->request->write($output);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_deleteSizeDetailByIds_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizedetail_ids = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizedetail_ids'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizedetail_ids'])){
				
				$this->sizedetail_ids = $vals['sizedetail_ids'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			
			$this->sizedetail_ids = array();
			$_size0 = 0;
			$input->readListBegin();
			while(true){
				
				try{
					
					$elem0 = null;
					$input->readI64($elem0); 
					
					$this->sizedetail_ids[$_size0++] = $elem0;
				}
				catch(\Exception $e){
					
					break;
				}
			}
			
			$input->readListEnd();
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizedetail_ids');
		
		if (!is_array($this->sizedetail_ids)){
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
		}
		
		$output->writeListBegin();
		foreach ($this->sizedetail_ids as $iter0){
			
			$xfer += $output->writeI64($iter0);
			
		}
		
		$output->writeListEnd();
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_deleteSizeTable_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizetable_id = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizetable_id'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizetable_id'])){
				
				$this->sizetable_id = $vals['sizetable_id'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			$input->readI64($this->sizetable_id); 
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizetable_id');
		$xfer += $output->writeI64($this->sizetable_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_deleteSizeTableTemplate_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $template_id = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'template_id'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['template_id'])){
				
				$this->template_id = $vals['template_id'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			$input->readI32($this->template_id); 
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('template_id');
		$xfer += $output->writeI32($this->template_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getCategorySizeTableConfigs_args {
	
	static $_TSPEC;
	public $third_category_ids = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'third_category_ids'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['third_category_ids'])){
				
				$this->third_category_ids = $vals['third_category_ids'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->third_category_ids = array();
			$_size0 = 0;
			$input->readSetBegin();
			while(true){
				
				try{
					
					$elem0 = null;
					$input->readI32($elem0); 
					
					$this->third_category_ids[$_size0++] = $elem0;
				}
				catch(\Exception $e){
					
					break;
				}
			}
			
			$input->readSetEnd();
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->third_category_ids !== null) {
			
			$xfer += $output->writeFieldBegin('third_category_ids');
			
			if (!is_array($this->third_category_ids)){
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeSetBegin();
			foreach ($this->third_category_ids as $iter0){
				
				$xfer += $output->writeI32($iter0);
				
			}
			
			$output->writeSetEnd();
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getDimensionInfos_args {
	
	static $_TSPEC;
	public $template_types = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'template_types'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['template_types'])){
				
				$this->template_types = $vals['template_types'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->template_types = array();
			$_size1 = 0;
			$input->readListBegin();
			while(true){
				
				try{
					
					$elem1 = null;
					$input->readI32($elem1); 
					
					$this->template_types[$_size1++] = $elem1;
				}
				catch(\Exception $e){
					
					break;
				}
			}
			
			$input->readListEnd();
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->template_types !== null) {
			
			$xfer += $output->writeFieldBegin('template_types');
			
			if (!is_array($this->template_types)){
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->template_types as $iter0){
				
				$xfer += $output->writeI32($iter0);
				
			}
			
			$output->writeListEnd();
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getSizeDetailList_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizetable_id = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizetable_id'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizetable_id'])){
				
				$this->sizetable_id = $vals['sizetable_id'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			$input->readI64($this->sizetable_id); 
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizetable_id');
		$xfer += $output->writeI64($this->sizetable_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getSizeDetailListByIds_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizedetail_ids = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizedetail_ids'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizedetail_ids'])){
				
				$this->sizedetail_ids = $vals['sizedetail_ids'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			
			$this->sizedetail_ids = array();
			$_size0 = 0;
			$input->readListBegin();
			while(true){
				
				try{
					
					$elem0 = null;
					$input->readI64($elem0); 
					
					$this->sizedetail_ids[$_size0++] = $elem0;
				}
				catch(\Exception $e){
					
					break;
				}
			}
			
			$input->readListEnd();
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizedetail_ids');
		
		if (!is_array($this->sizedetail_ids)){
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
		}
		
		$output->writeListBegin();
		foreach ($this->sizedetail_ids as $iter0){
			
			$xfer += $output->writeI64($iter0);
			
		}
		
		$output->writeListEnd();
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getSizeTableInfo_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizetable_id = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizetable_id'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizetable_id'])){
				
				$this->sizetable_id = $vals['sizetable_id'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			$input->readI64($this->sizetable_id); 
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizetable_id');
		$xfer += $output->writeI64($this->sizetable_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_healthCheck_args {
	
	static $_TSPEC;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			
			);
			
		}
		
		if (is_array($vals)){
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_querySizeTableTemplate_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $page = null;
	public $limit = null;
	public $sizetable_template_id = null;
	public $sizetable_template_name = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'page'
			),
			3 => array(
			'var' => 'limit'
			),
			4 => array(
			'var' => 'sizetable_template_id'
			),
			5 => array(
			'var' => 'sizetable_template_name'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['page'])){
				
				$this->page = $vals['page'];
			}
			
			
			if (isset($vals['limit'])){
				
				$this->limit = $vals['limit'];
			}
			
			
			if (isset($vals['sizetable_template_id'])){
				
				$this->sizetable_template_id = $vals['sizetable_template_id'];
			}
			
			
			if (isset($vals['sizetable_template_name'])){
				
				$this->sizetable_template_name = $vals['sizetable_template_name'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			$input->readI32($this->page); 
			
		}
		
		
		
		
		if(true) {
			
			$input->readI32($this->limit); 
			
		}
		
		
		
		
		if(true) {
			
			$input->readI32($this->sizetable_template_id); 
			
		}
		
		
		
		
		if(true) {
			
			$input->readString($this->sizetable_template_name);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		if($this->page !== null) {
			
			$xfer += $output->writeFieldBegin('page');
			$xfer += $output->writeI32($this->page);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->limit !== null) {
			
			$xfer += $output->writeFieldBegin('limit');
			$xfer += $output->writeI32($this->limit);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->sizetable_template_id !== null) {
			
			$xfer += $output->writeFieldBegin('sizetable_template_id');
			$xfer += $output->writeI32($this->sizetable_template_id);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->sizetable_template_name !== null) {
			
			$xfer += $output->writeFieldBegin('sizetable_template_name');
			$xfer += $output->writeString($this->sizetable_template_name);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_updateSizeDetail_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizedetail = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizedetail'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizedetail'])){
				
				$this->sizedetail = $vals['sizedetail'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			
			$this->sizedetail = new \Voop\vipapis\vsizetable\SizeDetailRequest();
			$this->sizedetail->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizedetail');
		
		if (!is_object($this->sizedetail)) {
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
		}
		
		$xfer += $this->sizedetail->write($output);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_updateSizeTableInfo_args {
	
	static $_TSPEC;
	public $vendor_id = null;
	public $sizetable = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			1 => array(
			'var' => 'vendor_id'
			),
			2 => array(
			'var' => 'sizetable'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['vendor_id'])){
				
				$this->vendor_id = $vals['vendor_id'];
			}
			
			
			if (isset($vals['sizetable'])){
				
				$this->sizetable = $vals['sizetable'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->vendor_id); 
			
		}
		
		
		
		
		if(true) {
			
			
			$this->sizetable = new \Voop\vipapis\vsizetable\SizeTableInfoRequest();
			$this->sizetable->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		$xfer += $output->writeFieldBegin('vendor_id');
		$xfer += $output->writeI32($this->vendor_id);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldBegin('sizetable');
		
		if (!is_object($this->sizetable)) {
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
		}
		
		$xfer += $this->sizetable->write($output);
		
		$xfer += $output->writeFieldEnd();
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_addSizeDetail_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = new \Voop\vipapis\vsizetable\SizeDetail();
			$this->success->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_object($this->success)) {
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->success->write($output);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_addSizeTableInfo_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = new \Voop\vipapis\vsizetable\SizeTableInfo();
			$this->success->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_object($this->success)) {
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->success->write($output);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_addSizeTableTemplate_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = new \Voop\vipapis\vsizetable\AddSizeTableTemplateResponse();
			$this->success->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_object($this->success)) {
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->success->write($output);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_deleteSizeDetailByIds_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = array();
			$_size0 = 0;
			$input->readListBegin();
			while(true){
				
				try{
					
					$elem0 = null;
					$input->readI64($elem0); 
					
					$this->success[$_size0++] = $elem0;
				}
				catch(\Exception $e){
					
					break;
				}
			}
			
			$input->readListEnd();
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_array($this->success)){
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->success as $iter0){
				
				$xfer += $output->writeI64($iter0);
				
			}
			
			$output->writeListEnd();
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_deleteSizeTable_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI64($this->success); 
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			$xfer += $output->writeI64($this->success);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_deleteSizeTableTemplate_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI32($this->success); 
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			$xfer += $output->writeI32($this->success);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getCategorySizeTableConfigs_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = array();
			$_size0 = 0;
			$input->readListBegin();
			while(true){
				
				try{
					
					$elem0 = null;
					
					$elem0 = new \Voop\vipapis\vsizetable\CategorySizetableConfig();
					$elem0->read($input);
					
					$this->success[$_size0++] = $elem0;
				}
				catch(\Exception $e){
					
					break;
				}
			}
			
			$input->readListEnd();
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_array($this->success)){
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->success as $iter0){
				
				
				if (!is_object($iter0)) {
					
					throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
				}
				
				$xfer += $iter0->write($output);
				
			}
			
			$output->writeListEnd();
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getDimensionInfos_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = new \Voop\vipapis\vsizetable\GetDimensionInfoResponse();
			$this->success->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_object($this->success)) {
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->success->write($output);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getSizeDetailList_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = array();
			$_size0 = 0;
			$input->readListBegin();
			while(true){
				
				try{
					
					$elem0 = null;
					
					$elem0 = new \Voop\vipapis\vsizetable\SizeDetail();
					$elem0->read($input);
					
					$this->success[$_size0++] = $elem0;
				}
				catch(\Exception $e){
					
					break;
				}
			}
			
			$input->readListEnd();
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_array($this->success)){
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->success as $iter0){
				
				
				if (!is_object($iter0)) {
					
					throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
				}
				
				$xfer += $iter0->write($output);
				
			}
			
			$output->writeListEnd();
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getSizeDetailListByIds_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = array();
			$_size1 = 0;
			$input->readListBegin();
			while(true){
				
				try{
					
					$elem1 = null;
					
					$elem1 = new \Voop\vipapis\vsizetable\SizeDetail();
					$elem1->read($input);
					
					$this->success[$_size1++] = $elem1;
				}
				catch(\Exception $e){
					
					break;
				}
			}
			
			$input->readListEnd();
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_array($this->success)){
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->success as $iter0){
				
				
				if (!is_object($iter0)) {
					
					throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
				}
				
				$xfer += $iter0->write($output);
				
			}
			
			$output->writeListEnd();
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_getSizeTableInfo_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = new \Voop\vipapis\vsizetable\SizeTableInfo();
			$this->success->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_object($this->success)) {
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->success->write($output);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_healthCheck_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = new \Voop\com\vip\hermes\core\health\CheckResult();
			$this->success->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_object($this->success)) {
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->success->write($output);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_querySizeTableTemplate_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			
			$this->success = new \Voop\vipapis\vsizetable\QuerySizeTableTemplateResponse();
			$this->success->read($input);
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			
			if (!is_object($this->success)) {
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->success->write($output);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_updateSizeDetail_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI64($this->success); 
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			$xfer += $output->writeI64($this->success);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




class VendorSizeTableService_updateSizeTableInfo_result {
	
	static $_TSPEC;
	public $success = null;
	
	public function __construct($vals=null){
		
		if (!isset(self::$_TSPEC)){
			
			self::$_TSPEC = array(
			0 => array(
			'var' => 'success'
			),
			
			);
			
		}
		
		if (is_array($vals)){
			
			
			if (isset($vals['success'])){
				
				$this->success = $vals['success'];
			}
			
			
		}
		
	}
	
	
	public function read($input){
		
		
		
		
		if(true) {
			
			$input->readI64($this->success); 
			
		}
		
		
		
		
		
		
	}
	
	public function write($output){
		
		$xfer = 0;
		$xfer += $output->writeStructBegin();
		
		if($this->success !== null) {
			
			$xfer += $output->writeFieldBegin('success');
			$xfer += $output->writeI64($this->success);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		$xfer += $output->writeFieldStop();
		$xfer += $output->writeStructEnd();
		return $xfer;
	}
	
}




?>