...
class EnterpriseOrderResponse {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['enterpriseOrders'])){
				
				$this->enterpriseOrders = $vals['enterpriseOrders'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("enterpriseOrders" == $schemeField){
				
				$needSkip = false;
				
				$this->enterpriseOrders = array();
				$_size1 = 0;
				$input->readListBegin();
				while(true){
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('enterpriseOrders');
		...
		foreach ($this->enterpriseOrders as $iter0){
