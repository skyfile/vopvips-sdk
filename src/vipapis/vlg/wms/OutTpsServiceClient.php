...
class OutTpsServiceClient{
	...
	public static function getService(){
		...
		if (! OutTpsServiceClient::$isRegis){
			
			OutTpsServiceClient::regis ();
			OutTpsServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = OutTpsServiceClient::$DEFAULT_PROXY_IP;
		$port = OutTpsServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\vlg\wms\OutTpsServiceHelper();
	}
