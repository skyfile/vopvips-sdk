...
class CarrierLogisticTrackServiceClient{
	...
	public static function getService(){
		...
		if (! CarrierLogisticTrackServiceClient::$isRegis){
			
			CarrierLogisticTrackServiceClient::regis ();
			CarrierLogisticTrackServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = CarrierLogisticTrackServiceClient::$DEFAULT_PROXY_IP;
		$port = CarrierLogisticTrackServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\track\service\CarrierLogisticTrackServiceHelper();
	}
