...
class InternalInventoryServiceClient{
	...
	public static function getService(){
		...
		if (! InternalInventoryServiceClient::$isRegis){
			
			InternalInventoryServiceClient::regis ();
			InternalInventoryServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = InternalInventoryServiceClient::$DEFAULT_PROXY_IP;
		$port = InternalInventoryServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\marketplace\inventory\InternalInventoryServiceHelper();
	}
