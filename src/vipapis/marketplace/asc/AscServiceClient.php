...
class AscServiceClient{
	...
	public static function getService(){
		...
		if (! AscServiceClient::$isRegis){
			
			AscServiceClient::regis ();
			AscServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = AscServiceClient::$DEFAULT_PROXY_IP;
		$port = AscServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\marketplace\asc\AscServiceHelper();
	}
