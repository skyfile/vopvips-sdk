...
class StoreSizeTableServiceClient{
	...
	public static function getService(){
		...
		if (! StoreSizeTableServiceClient::$isRegis){
			
			StoreSizeTableServiceClient::regis ();
			StoreSizeTableServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = StoreSizeTableServiceClient::$DEFAULT_PROXY_IP;
		$port = StoreSizeTableServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\marketplace\sizetable\StoreSizeTableServiceHelper();
	}
