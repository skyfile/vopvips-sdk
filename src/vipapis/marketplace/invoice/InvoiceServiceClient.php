...
class InvoiceServiceClient{
	...
	public static function getService(){
		...
		if (! InvoiceServiceClient::$isRegis){
			
			InvoiceServiceClient::regis ();
			InvoiceServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = InvoiceServiceClient::$DEFAULT_PROXY_IP;
		$port = InvoiceServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\marketplace\invoice\InvoiceServiceHelper();
	}
