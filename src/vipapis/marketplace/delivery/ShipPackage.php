...
class ShipPackage {
	...
	public $package_product_list = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['package_product_list'])){
				
				$this->package_product_list = $vals['package_product_list'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("package_product_list" == $schemeField){
				
				$needSkip = false;
				
				$this->package_product_list = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem0 = new \Voop\vipapis\marketplace\delivery\ShipPackageProduct();
						$elem0->read($input);
						
						$this->package_product_list[$_size0++] = $elem0;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('package_product_list');
		
		if (!is_array($this->package_product_list)){
			
			throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
		}
		
		$output->writeListBegin();
		foreach ($this->package_product_list as $iter0){
