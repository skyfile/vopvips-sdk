...
class DecryptData {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['decrypt_items'])){
				
				$this->decrypt_items = $vals['decrypt_items'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("decrypt_items" == $schemeField){
				
				$needSkip = false;
				
				$this->decrypt_items = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('decrypt_items');
		...
		foreach ($this->decrypt_items as $iter0){
