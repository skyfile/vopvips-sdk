...
class StockServiceClient{
	...
	public static function getService(){
		...
		if (! StockServiceClient::$isRegis){
			
			StockServiceClient::regis ();
			StockServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = StockServiceClient::$DEFAULT_PROXY_IP;
		$port = StockServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\stock\StockServiceHelper();
	}
