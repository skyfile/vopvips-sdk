...
class SalesServiceClient{
	...
	public static function getService(){
		...
		if (! SalesServiceClient::$isRegis){
			
			SalesServiceClient::regis ();
			SalesServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = SalesServiceClient::$DEFAULT_PROXY_IP;
		$port = SalesServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\sales\SalesServiceHelper();
	}
