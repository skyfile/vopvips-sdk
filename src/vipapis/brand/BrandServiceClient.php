...
class BrandServiceClient{
	...
	public static function getService(){
		...
		if (! BrandServiceClient::$isRegis){
			
			BrandServiceClient::regis ();
			BrandServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = BrandServiceClient::$DEFAULT_PROXY_IP;
		$port = BrandServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\brand\BrandServiceHelper();
	}
