...
class JitXServiceClient{
	...
	public static function getService(){
		...
		if (! JitXServiceClient::$isRegis){
			
			JitXServiceClient::regis ();
			JitXServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = JitXServiceClient::$DEFAULT_PROXY_IP;
		$port = JitXServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\jitx\JitXServiceHelper();
	}
