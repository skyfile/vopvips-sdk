...
class DeliveryOrder {
	...
	public $delivery_order_details = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['delivery_order_details'])){
				
				$this->delivery_order_details = $vals['delivery_order_details'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("delivery_order_details" == $schemeField){
				
				$needSkip = false;
				
				$this->delivery_order_details = array();
				$_size1 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem1 = new \Voop\vipapis\jitx\DeliveryOrderDetail();
						$elem1->read($input);
						
						$this->delivery_order_details[$_size1++] = $elem1;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->delivery_order_details !== null) {
			
			$xfer += $output->writeFieldBegin('delivery_order_details');
			
			if (!is_array($this->delivery_order_details)){
				
				throw new \Voop\Osp\Exception\OspException('Bad type in structure.', \Voop\Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->delivery_order_details as $iter0){
