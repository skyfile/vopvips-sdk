...
class ProductInventoryServiceClient{
	...
	public static function getService(){
		...
		if (! ProductInventoryServiceClient::$isRegis){
			
			ProductInventoryServiceClient::regis ();
			ProductInventoryServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = ProductInventoryServiceClient::$DEFAULT_PROXY_IP;
		$port = ProductInventoryServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\normal\ProductInventoryServiceHelper();
	}
