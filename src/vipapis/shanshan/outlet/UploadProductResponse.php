...
class UploadProductResponse {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['successes'])){
				
				$this->successes = $vals['successes'];
			}
			...
			if ("successes" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem0 = new \Voop\vipapis\shanshan\outlet\UploadProductResult();
						$elem0->read($input);
			...
			if ("failures" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem1 = new \Voop\vipapis\shanshan\outlet\UploadProductResult();
						$elem1->read($input);
	...
	public function write($output){
		...
		if($this->successes !== null) {
			...
			$xfer += $output->writeFieldBegin('successes');
			...
			foreach ($this->successes as $iter0){
