...
class ShanshanOutletServiceClient{
	...
	public static function getService(){
		...
		if (! ShanshanOutletServiceClient::$isRegis){
			
			ShanshanOutletServiceClient::regis ();
			ShanshanOutletServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = ShanshanOutletServiceClient::$DEFAULT_PROXY_IP;
		$port = ShanshanOutletServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\shanshan\outlet\ShanshanOutletServiceHelper();
	}
