...
class AfterSaleServiceClient{
	...
	public static function getService(){
		...
		if (! AfterSaleServiceClient::$isRegis){
			
			AfterSaleServiceClient::regis ();
			AfterSaleServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = AfterSaleServiceClient::$DEFAULT_PROXY_IP;
		$port = AfterSaleServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\cup\aftersale\AfterSaleServiceHelper();
	}
