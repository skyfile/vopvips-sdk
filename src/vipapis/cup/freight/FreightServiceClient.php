...
class FreightServiceClient{
	...
	public static function getService(){
		...
		if (! FreightServiceClient::$isRegis){
			
			FreightServiceClient::regis ();
			FreightServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Voop\Osp\Context\InvocationContextFactory::getInstance ();
		$ip = FreightServiceClient::$DEFAULT_PROXY_IP;
		$port = FreightServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\cup\freight\FreightServiceHelper();
	}
