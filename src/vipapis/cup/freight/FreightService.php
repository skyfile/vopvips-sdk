...
interface FreightServiceIf{
	...
	public function getOrderFreight( $get_order_freight_req_list);
...
class _FreightServiceClient extends \Osp\Base\OspStub implements \vipapis\cup\freight\FreightServiceIf{
	...
	public function __construct(){
		...
		parent::__construct("vipapis.cup.freight.FreightService", "1.0.0");
	}
	
	
	public function getOrderFreight( $get_order_freight_req_list){
		
		$this->send_getOrderFreight( $get_order_freight_req_list);
		return $this->recv_getOrderFreight();
	}
...
class FreightService_getOrderFreight_args {
	
	static $_TSPEC;
	public $get_order_freight_req_list = null;
	...
	public function __construct($vals=null){
		...
		if (!isset(self::$_TSPEC)){
			...
			self::$_TSPEC = array(
			1 => array(
			'var' => 'get_order_freight_req_list'
			),
		...
		if (is_array($vals)){
			...
			if (isset($vals['get_order_freight_req_list'])){
				
				$this->get_order_freight_req_list = $vals['get_order_freight_req_list'];
			}
	...
	public function read($input){
		...
		if(true) {
			...
			$this->get_order_freight_req_list = array();
			$_size1 = 0;
			$input->readListBegin();
			while(true){
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('get_order_freight_req_list');
		...
		foreach ($this->get_order_freight_req_list as $iter0){
...
class FreightService_healthCheck_args {
...
class FreightService_getOrderFreight_result {
...
class FreightService_healthCheck_result {
