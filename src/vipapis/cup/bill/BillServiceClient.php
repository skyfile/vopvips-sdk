...
class BillServiceClient{
	...
	public static function getService(){
		...
		if (! BillServiceClient::$isRegis){
			
			BillServiceClient::regis ();
			BillServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = BillServiceClient::$DEFAULT_PROXY_IP;
		$port = BillServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\cup\bill\BillServiceHelper();
	}
