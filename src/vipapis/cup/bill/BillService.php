...
class _BillServiceClient extends \Voop\Osp\Base\OspStub implements \Voop\vipapis\cup\bill\BillServiceIf{
	...
	public function __construct(){
		...
		parent::__construct("vipapis.cup.bill.BillService", "1.0.0");
	}
...
class BillService_healthCheck_args {
...
class BillService_updateBillInfo_args {
	
	static $_TSPEC;
	public $bill_header_info = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['bill_header_info'])){
				
				$this->bill_header_info = $vals['bill_header_info'];
			}
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('bill_header_info');
...
class BillService_healthCheck_result {
...
class BillService_updateBillInfo_result {
