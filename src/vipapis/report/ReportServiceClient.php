...
class ReportServiceClient{
	...
	public static function getService(){
		...
		if (! ReportServiceClient::$isRegis){
			
			ReportServiceClient::regis ();
			ReportServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = ReportServiceClient::$DEFAULT_PROXY_IP;
		$port = ReportServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \Voop\vipapis\report\ReportServiceHelper();
	}
